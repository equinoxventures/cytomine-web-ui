...
<template>
<cytomine-modal-card :title="$t('add-attached-snapshot')">
  <b-field>
  ...
  methods: {
    async save() {
      ...
      try {
        ...
        console.log(error);
        this.$notify({type: 'error', text: this.$t('notif-error-attached-snapshot-creation')});
      }
    },
  }

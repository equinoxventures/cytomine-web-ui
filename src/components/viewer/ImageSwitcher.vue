...
<template>
<div>
  <div class="image-switcher-wrapper" v-show="imageSwitcherEnabled">
    ...
    <template v-if="!loading">
      <div class="header">
        ...
        <button class="delete" @click="imageSwitcherEnabled = false"></button>
      </div>
      ...
      <div v-else class="image-switcher" ref="imageSelector">
  ...
  <a
    class="image-switcher-button"
    @click="imageSwitcherEnabled = true"
    v-tooltip="{content: $t('Toggle image'), placement: 'left'}"
  >
    <i>
...
export default {
  ...
  computed: {
    ...
    imageSwitcherEnabled: {
      get() {
        return this.$store.getters['currentProject/currentViewer'].ImageSwitcher;
      },
      set(value) {
        this.$store.commit(this.viewerModule + 'setImageSwitcher', value);
      }
    },
    ...
    activeImage(){
      return  this.viewerWrapper.activeImage;
    },
  },
  watch:{
    async selectedTags(){
  ...
  methods: {
    async addImage(image) {
      try {
        ...
        await this.$store.dispatch(this.viewerModule + 'removeImage',this.activeImage);
        await this.$store.dispatch(this.viewerModule + 'addImage', {image, slice});
      }
      catch(error) {
        console.log(error);
        this.$notify({type: 'error', text: this.$t('No active image, Please click on the main view you want to replace')});
      }
    },
    async fetchImages() {
    ...
    toggle() {
      this.imageSwitcherEnabled = !this.imageSwitcherEnabled;
    },
  ...
  mounted() {
    ...
    this.$refs.imageSelector.addEventListener('wheel', this.scrollHorizontally);
  },
  beforeDestroy() {
    this.$eventBus.$off('shortkeyEvent', this.shortkeyHandler);
    this.$refs.imageSelector.removeEventListener('wheel', this.scrollHorizontally);
  }
};
</script>
